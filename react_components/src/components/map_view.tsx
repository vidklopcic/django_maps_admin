    ...
    return <MapStyles color={store.lighterColor}>
        <MapContainer bounds={store.bounds} zoomControl={false}
            ...
            {store.points.length && <Polyline
                ...
                eventHandlers={{
                    click: (e) => {
                        store.addPoint(MapUtils.getSegment(e.latlng, e.sourceTarget.getLatLngs(), e.sourceTarget._map), e.latlng);
    ...
    return <MapStyles color={store.lighterColor}>
        <MapContainer bounds={store.bounds} zoomControl={false}
            ...
            {store.points.length && <Polygon
                ...
                eventHandlers={{
                    click: (e) => {
                        store.addPoint(MapUtils.getSegment(e.latlng, e.sourceTarget.getLatLngs()[0], e.sourceTarget._map), e.latlng);
                        throw 'stopped propagation';
                    }
                }}/>}
            {markers}
